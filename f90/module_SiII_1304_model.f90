module module_SiII_1304_model

  ! This module describes the absorption of photons by SiII from level  3s^2 3p 2P^0 1/2 to level 3s 3p^2 2S 1/2.
  ! This transition is at 1304.37 A.
  ! The module also implements the two decay channels (resonant and fluorescent) at 1304.37 A and 1309.27 A. 
  ...
  ! level 4 is 3s 3p^2 2S 1/2

  ! transition between levels 1 and 4
  real(kind=8),parameter :: lambda14       = 1304.37d0                    ! transition wavelength [A]
  real(kind=8),parameter :: lambda14_cm    = lambda14 / cmtoA             ! [cm]
  real(kind=8),parameter :: nu14           = clight / lambda14_cm         ! [Hz]
  real(kind=8),parameter :: f14            = 9.28d-2                      ! oscillator strength
  real(kind=8),parameter :: sigma14_factor = sqrtpi*e_ch**2*f14/me/clight ! multiply by Voigt(x,a)/delta_nu_doppler to get sigma.
  real(kind=8),parameter :: A41            = 3.64d8                       ! spontaneous decay [/s]

  ! transition between levels 2 and 4
  real(kind=8),parameter :: lambda24       = 1309.27d0                    ! transition wavelength [A]
  real(kind=8),parameter :: lambda24_cm    = lambda24 / cmtoA             ! [cm]
  real(kind=8),parameter :: nu24           = clight / lambda24_cm         ! [Hz]
  real(kind=8),parameter :: A42            = 6.23d8                       ! spontaneous decay [/s]

  real(kind=8),parameter :: Atot = A41+A42

  public :: get_tau_SiII_1304, scatter_SiII_1304, SiII_1304_peeloff_weight, read_SiII_1304_params, print_SiII_1304_params

contains

  function get_tau_SiII_1304(nSiII, vth, distance_to_border_cm, nu_cell)

    ! --------------------------------------------------------------------------
    ! compute optical depth of SiII-1304.37 over a given distance
    ! --------------------------------------------------------------------------
    ! INPUTS:
    ...
    ! OUTPUT :
    ! - get_tau_SiII_1304 : optical depth of Silicon's line over distance_to_border_cm
    ! --------------------------------------------------------------------------
    
    real(kind=8),intent(in) :: nSiII,vth,distance_to_border_cm,nu_cell
    real(kind=8)            :: delta_nu_doppler,x_cell,sigma,a,h_cell,get_tau_SiII_1304

    ! compute Doppler width and a-parameter
    delta_nu_doppler = vth / lambda14_cm
    a = A41 / (fourpi *  delta_nu_doppler)

    ! cross section of SiII-1304.37
    x_cell = (nu_cell - nu14) / delta_nu_doppler
    h_cell = voigt_function(x_cell,a)
    sigma  = sigma14_factor / delta_nu_doppler * h_cell

    get_tau_SiII_1304 = sigma * nSiII * distance_to_border_cm
   
    return

  end function get_tau_SiII_1304

  
  subroutine scatter_SiII_1304(vcell,vth,nu_cell,k,nu_ext,iran)
  ...
  end subroutine scatter_SiII_1304


  !--PEEL--
  function SiII_1304_peeloff_weight(vcell,vth,nu_ext,kin,kout,iran)
    ...
    real(kind=8),intent(in)                 :: vth
    integer(kind=4),intent(inout)           :: iran
    real(kind=8)                            :: SiII_1304_peeloff_weight
    real(kind=8)                            :: delta_nu_doppler, a, x_cell, upar, ruper
    real(kind=8)                            :: r2, uper, nu_atom, mu, bu, scalar, proba41
    ...
    ! 4/ determine direction of scattered photon
    SiII_1304_peeloff_weight = 0.5d0  ! P(mu) for isotropic phase function
  ...
  end function SiII_1304_peeloff_weight
!--LEEP--


  subroutine read_SiII_1304_params(pfile)
  ...
  end subroutine read_SiII_1304_params


    subroutine print_SiII_1304_params(unit)
  ...
  end subroutine print_SiII_1304_params


end module module_SiII_1304_model
