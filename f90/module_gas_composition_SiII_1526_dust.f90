module module_gas_composition
  ...
  ! This modules handles one transition in absorption at 1526.72A and dust

  use module_SiII_1526_model
...
contains
  ...
  subroutine gas_from_ramses_leaves_ions(repository,snapnum,nleaf,nvar,ramses_var,ion_number,SiII_density,g)
    ...
    if(ion_number /= 1) then
       print*, 'Error in module_gas_composition_SiII_1526_dust.f90,  the number of ions in ion_parameter_file should be 1.'
       stop
    end if
    ...
    if (gas_overwrite) then
       ...
       write(*,*) '-- module_gas_composition_SiII_1526_dust : extracting velocities from ramses '
       allocate(v(3,nleaf))
       ...
       write(*,*) '-- module_gas_composition_SiII_1526_dust : extracting nHI and T from ramses '
       ...
       write(*,*) '-- module_gas_composition_SiII_1526_dust : extracting ndust from ramses '
  ...
  function  gas_get_scatter_flag(cell_gas, distance_to_border_cm, nu_cell, tau_abs,iran)
    ...
    ! - gas_get_scatter_flag : 0 [no scatter], 1 [SiII-1526 scatter], 2 [dust] 
    ! --------------------------------------------------------------------------
    ...
    real(kind=8)                          :: tau_SiII_1526, tau_dust, tau_cell, proba60, x
    
    ! compute optical depths for different components of the gas.
    tau_SiII_1526 = get_tau_SiII_1526(cell_gas%nSiII, cell_gas%dopwidth, distance_to_border_cm, nu_cell)
    tau_dust      = get_tau_dust(cell_gas%ndust, distance_to_border_cm, nu_cell)
    tau_cell      = tau_SiII_1526 + tau_dust
    ...
    if (tau_abs > tau_cell) then  ! photon is due for absorption outside the cell 
       ...
       proba60 = tau_SiII_1526 /  tau_cell
       x = ran3(iran)
       if (x <= proba60) then
  ...
  function  gas_get_tau(cell_gas, distance_cm, nu_cell)
    ...
    ! compute optical depths for different components of the gas.
    tau_SiII   = get_tau_SiII_1526(cell_gas%nSiII, cell_gas%dopwidth, distance_cm, nu_cell)
  ...
  function gas_peeloff_weight(flag,cell_gas,nu_ext,kin,kout,iran)
    ...
    select case(flag)
    case(1)
       gas_peeloff_weight = SiII_1526_peeloff_weight(cell_gas%v, cell_gas%dopwidth, nu_ext, kin, kout, iran)
    case(2)
  ...
  !--PIKSEROC--
    ...
    select case(flag)
    case(1)
       call scatter_SiII_1526(cell_gas%v, cell_gas%dopwidth, nu_cell, k, nu_ext, iran)
    case(2)
...
subroutine read_gas(unit,n,g)
  ...
  subroutine read_gas_composition_params(pfile)
    ...
    call read_SiII_1526_params(pfile)
  ...
  subroutine print_gas_composition_params(unit)
    ...
    if (present(unit)) then 
       ...
       write(*,'(a)')             ' '
       call print_SiII_1526_params(unit)
