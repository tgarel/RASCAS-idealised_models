module module_CII_1334_model


  ! This module describes the absorption of photons by CII from level  2s^2 2p 2P^0 1/2 to level 2s2p^2 2D 3/2.
  ! This transition is at 1334.53 A.
  ! The module also implements the two decay channels (resonant and fluorescent) at 1334.53 A and 1335.66 A. 
  ...
  ! level 1 is 2s^2 2p 2P^0 1/2
  ! level 2 is 2s2p^2 2D 3/2
  ! level 3 is 2s^2 2p 2P^0 3/2

  ! transition between levels 1 and 3 

  real(kind=8),parameter :: lambda13       = 1334.53d0                    ! transition wavelength [A]
  real(kind=8),parameter :: lambda13_cm    = lambda13 / cmtoA             ! [cm]
  real(kind=8),parameter :: nu13           = clight / lambda13_cm         ! [Hz]
  real(kind=8),parameter :: f13            = 0.129d0                      ! oscillator strength
  real(kind=8),parameter :: sigma13_factor = sqrtpi*e_ch**2*f13/me/clight ! multiply by Voigt(x,a)/delta_nu_doppler to get sigma.
  real(kind=8),parameter :: A31            = 2.41d8                       ! spontaneous decay [/s]

  ! transition between levels 2 and 3
  real(kind=8),parameter :: lambda23       = 1335.66d0                    ! transition wavelength [A]
  real(kind=8),parameter :: lambda23_cm    = lambda23 / cmtoA             ! [cm]
  real(kind=8),parameter :: nu23           = clight / lambda23_cm         ! [Hz]
  real(kind=8),parameter :: A32            = 4.76d7                       ! spontaneous decay [/s]
    
  real(kind=8),parameter :: Atot = A31+A32


  public :: get_tau_CII_1334, scatter_CII_1334, read_CII_1334_params, print_CII_1334_params

  
contains

  function get_tau_CII_1334(nCII, vth, distance_to_border_cm, nu_cell)

    ! --------------------------------------------------------------------------
    ! compute optical depth of CII-1334.53 over a given distance
    ! --------------------------------------------------------------------------
    ! INPUTS:
    ! - nCII    : number density of CII ions                              [ cm^-3 ]
    ! - vth      : thermal (+ small-scale turbulence) velocity of CII ions [ cm / s ]
    ! - distance_to_border_cm : distance over which we compute tau          [ cm ]
    ! - nu_cell  : photon's frequency in the frame of the cell              [ Hz ]
    ! OUTPUT :
    ! - get_tau_CII_1334 : optical depth of Carbon's line over distance_to_border_cm
    ! --------------------------------------------------------------------------
    
    real(kind=8),intent(in) :: nCII,vth,distance_to_border_cm,nu_cell
    real(kind=8)            :: delta_nu_doppler,x_cell,sigma,a,h_cell,get_tau_CII_1334

    ! compute Doppler width and a-parameter
    delta_nu_doppler = vth / lambda13_cm
    a = A31 / (fourpi * delta_nu_doppler)

    ! cross section of CII-1334.53
    x_cell = (nu_cell - nu13) / delta_nu_doppler
    h_cell = voigt_function(x_cell,a)
    sigma  = sigma13_factor / delta_nu_doppler * h_cell

    get_tau_CII_1334 = sigma * nCII * distance_to_border_cm
   
    return
    
  end function get_tau_CII_1334


  subroutine scatter_CII_1334(vcell,vth,nu_cell,k,nu_ext,iran)

    ! ---------------------------------------------------------------------------------
    ! perform scattering event on a CII ion
    ...
    ! - vth      : thermal (+turbulent) velocity dispersion of H (and C) atoms [ cm / s ] 
  ...
  end subroutine scatter_CII_1334

  
  subroutine read_CII_1334_params(pfile)
  ...
  end subroutine read_CII_1334_params


  subroutine print_CII_1334_params(unit)
  ...
  end subroutine print_CII_1334_params


end module module_CII_1334_model
