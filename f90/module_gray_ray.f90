module module_gray_ray
  ...
  public  :: init_rays_from_file, init_halos_from_file, dump_rays, column_density!ray_advance, 
  private :: path

contains

  
  subroutine ray_advance(ray,domesh,domaine_calcul,dist_tot)
    ...
    real(kind=8),intent(in)        :: dist_tot       ! stop propagation after a distance of dist_tot
    ...
    dist_tot_cm = dist_tot ! dist_tot is now provided in cm ... 
    ...
    ray_propagation : do
       ...
       ! Check if dist_tot is atteined within the cell. If so, stops the propagation in the middle of the cell
       if(dist + distance_to_border_cm > dist_tot) then
          ray%halo_ID = 1
          d = sqrt((ppos_cell(1)-5d-1)**2 + (ppos_cell(2)-5d-1)**2 + (ppos_cell(3)-5d-1)**2)*cell_size_cm
          dist = dist + d
          tau = tau + gas_get_tau_gray(cell_gas, d)
          ray%dist = dist
          ray%tau = tau
