module module_gas_composition
  ...
  character(100),parameter :: moduleName = 'module_gas_composition_OI_1302_SiII_1304_dust.f90'
  type, public :: gas
     ! fluid
...
contains
  ...
  subroutine gas_from_ramses_leaves_ions(repository,snapnum,nleaf,nvar,ramses_var,ion_number,ion_densities,g)
    ...
    if(ion_number /= 2) then
       print*, 'Error in module_gas_composition_OI_1302_SiII_1304_dust.f90,  the number of ions in ion_parameter_file should be 2.'
       stop
    end if
    ...
    if (gas_overwrite) then
       ...
       write(*,*) '-- module_gas_composition_OI_1302_SiII_1304_dust : extracting velocities from ramses '
       allocate(v(3,nleaf))
       ...
       write(*,*) '-- module_gas_composition_OI_1302_SiII_1304_dust : extracting nHI and T from ramses '
       ...
       write(*,*) '-- module_gas_composition_OI_1302_SiII_1304_dust : extracting ndust from ramses '
  ...
  function  gas_get_scatter_flag(cell_gas, distance_to_border_cm, nu_cell, tau_abs,iran)
    ...
    if (tau_abs > tau_cell) then  ! photon is due for absorption outside the cell 
       ...
       ! decide if it is 1302 or 1304 or dust 
       proba02 = tau_OI_1302 /  tau_cell
       proba04 = proba02 + tau_SiII_1304 / tau_cell 
       x = ran3(iran)
       if (x <= proba02) then
          gas_get_scatter_flag = 1 ! absorption by OI-1302
       else if (x <= proba04) then
  ...
  function  gas_get_tau(cell_gas, distance_cm, nu_cell)
    ...
    ! --------------------------------------------------------------------------
    ! INPUTS:
    ! - cell_gas : a mix of OI, SiII and dust
  ...
  function gas_get_CD(cell_gas, distance_cm)
    ...
    real(kind=8), intent(in) :: distance_cm
    type(gas),intent(in)     :: cell_gas
    real(kind=8)             :: gas_get_CD(3)

    gas_get_CD = (/ distance_cm*cell_gas%nOI, distance_cm*cell_gas%nSiII, distance_cm*cell_gas%ndust /)
