...
!!$       close(15)
  ...
  subroutine overwrite_gas(g,x_leaf,leaf_level,nleaf)
    ...
    case('slab_plus_sphere')      
       do ileaf=1,nleaf
          dx_cell = 0.5d0**leaf_level(ileaf)
          call slab_plus_sphere(g(ileaf)%v(1),g(ileaf)%v(2),g(ileaf)%v(3),g(ileaf)%nSiII,g(ileaf)%dopwidth,x_leaf(ileaf,1),x_leaf(ileaf,2),x_leaf(ileaf,3),dx_cell,g(ileaf)%ndust)
       end do
