module module_gas_composition
  ...
  ! Mix of AlII and dust 
  ! This modules handles one transition in absorption at 1670 A and dust

  use module_AlII_1670_model
  ...
  type, public :: gas
     ...
     ! AlII
     real(kind=8) :: nAlII     ! numerical density of AlII  [#/cm3]
  ...
  character(2000)          :: Ion_file           !Path to the file with the AlII densities
  ...
  !real(kind=8)             :: vturb           = 2d1    ! Constant turbulent velocity over all simulation,  in km/s
  ...
  real(kind=8)             :: fix_nAlII           = 1.0d0   ! ad-hoc AlII density (1/cm3)
...
contains
  ...
  subroutine gas_from_list(ncells, cell_pos, cell_l, gas_leaves)
    ...
    gas_leaves%dopwidth = gas_leaves%dopwidth / sqrt(mAl/amu)  ! remove for 2*2*2 test
    !read(20) gas_leaves%ndust !add for 2*2*2 test
    !read(20) gas_leaves%nAlII !add for 2*2*2 test
    read(21) gas_leaves%nAlII !remove for 2*2*2 test
    ...
    if (verbose) print*,'min/max of nAlII : ',minval(gas_leaves(:)%nAlII),maxval(gas_leaves(:)%nAlII)
  ...
  !Val--
  ! --------------------------------------------------------------------------
  subroutine gas_from_ramses_leaves_ions(repository,snapnum,nleaf,nvar,ramses_var,ion_number,AlII_density,g)
    ...
    real(kind=8),intent(in),dimension(ion_number,nleaf) :: AlII_density
    ...
    if(ion_number /= 1) then
       print*, 'Error in module_gas_composition_AlII_1670_dust.f90,  the number of ions in ion_parameter_file should be 1.'
       stop
    end if
    ...
    if (gas_overwrite) then
       ...
       write(*,*) '-- module_gas_composition_AlII_1670_dust : extracting velocities from ramses '
       allocate(v(3,nleaf))
       ...
       write(*,*) '-- module_gas_composition_AlII_1670_dust : extracting nHI and T from ramses '
       ...
       g(:)%dopwidth = sqrt((2.0d0*kb/mAl)*T)!  + 2d6) ! [ cm/s ]    ! Added a 20 km/s constant turbulent velocity on all the simulation

       ! get ndust (pseudo dust density from Laursen, Sommer-Larsen, Andersen 2009)
       write(*,*) '-- module_gas_composition_AlII_1670_dust : extracting ndust from ramses '
  ...
  subroutine gas_from_ramses_leaves(repository,snapnum,nleaf,nvar,ramses_var, g)
    ...
    type(gas),dimension(:),allocatable,intent(out) :: g
    integer(kind=4)                   :: ileaf
    real(kind=8),allocatable          :: v(:,:), T(:), nAlII(:), nHI(:), nHII(:), metallicity(:)
    ...
    if (gas_overwrite) then
       ...
       print*,'Cannot extract nAlII directly from Ramses. Please use gas_from_ramses_leaves_ions or gas_from_list instead of gas_from_ramses_leaves.'
       stop
  ...
  function gas_get_CD(cell_gas, distance_cm)
    ...
    gas_get_CD = (/ distance_cm*cell_gas%nAlII, distance_cm*cell_gas%ndust /)
  ...
  function  gas_get_scatter_flag(cell_gas, distance_to_border_cm, nu_cell, tau_abs,iran)
    ...
    ! --------------------------------------------------------------------------
    ! INPUTS:
    ! - cell_gas : AlII (with two absorption channels) and dust
    ...
    ! - gas_get_scatter_flag : 0 [no scatter], 1 [AlII-1670 scatter], 2 [dust] 
    ! --------------------------------------------------------------------------
    ...
    real(kind=8)                          :: tau_AlII_1670, tau_dust, tau_cell, proba60, x
    
    ! compute optical depths for different components of the gas.
    tau_AlII_1670 = get_tau_AlII_1670(cell_gas%nAlII, cell_gas%dopwidth, distance_to_border_cm, nu_cell)
    tau_dust      = get_tau_dust(cell_gas%ndust, distance_to_border_cm, nu_cell)
    tau_cell      = tau_AlII_1670 + tau_dust
    ...
    if (tau_abs > tau_cell) then  ! photon is due for absorption outside the cell 
       ...
       ! decide if it is 1670 or dust 
       proba60 = tau_AlII_1670 /  tau_cell
       x = ran3(iran)
       if (x <= proba60) then
          gas_get_scatter_flag = 1 ! absorption by AlII-1670
       else
  ...
  function  gas_get_tau(cell_gas, distance_cm, nu_cell)
    ...
    ! --------------------------------------------------------------------------
    ! INPUTS:
    ! - cell_gas : a mix of AlII and dust
    ...
    real(kind=8)            :: tau_AlII, tau_dust

    ! compute optical depths for different components of the gas.
    tau_AlII   = get_tau_AlII_1670(cell_gas%nAlII, cell_gas%dopwidth, distance_cm, nu_cell)
    tau_dust = get_tau_dust(cell_gas%ndust, distance_cm, nu_cell)
    gas_get_tau = tau_AlII + tau_dust
  ...
  function gas_peeloff_weight(flag,cell_gas,nu_ext,kin,kout,iran)
    ...
    select case(flag)
    case(1)
       gas_peeloff_weight = AlII_1670_peeloff_weight(cell_gas%v, cell_gas%dopwidth, nu_ext, kin, kout, iran)
    case(2)
       gas_peeloff_weight = dust_peeloff_weight(cell_gas%v, nu_ext, kin, kout)
    case default
       print*,'ERROR in module_gas_composition_AlII1670_dust.f90:gas_peeloff_weight - unknown case : ',flag 
       stop
    end select
  ...
  !--PIKSEROC--
    ...
    select case(flag)
    case(1)
       call scatter_AlII_1670(cell_gas%v, cell_gas%dopwidth, nu_cell, k, nu_ext, iran)
    case(2)
  ...
  subroutine dump_gas(unit,g)
    ...
    write(unit) (g(i)%nAlII, i=1,nleaf)
...
subroutine read_gas(unit,n,g)
    ...
    if (verbose) print*, 'min/max of nAlII : ',minval(g(:)%nAlII),maxval(g(:)%nAlII)
  ...
  subroutine read_gas_composition_params(pfile)
    ...
    call read_AlII_1670_params(pfile)
  ...
  subroutine print_gas_composition_params(unit)
    ...
    if (present(unit)) then 
       ...
       write(unit,'(a,ES10.3)') '  fix_nAlII            = ',fix_nAlII
       ...
       write(unit,'(a)')             ' '
       call print_AlII_1670_params(unit)
