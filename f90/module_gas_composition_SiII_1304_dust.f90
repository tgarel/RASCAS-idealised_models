module module_gas_composition
  ...
  ! This modules handles one transition in absorption at 1304 A and dust
...
contains
  ...
  subroutine gas_from_ramses_leaves_ions(repository,snapnum,nleaf,nvar,ramses_var,ion_number,SiII_density,g)
    ...
    if(ion_number /= 1) then
       print*, 'Error in module_gas_composition_SiII_1304_dust.f90,  the number of ions in ion_parameter_file should be 1.'
       stop
    end if
    ...
    if (gas_overwrite) then
       ...
       write(*,*) '-- module_gas_composition_SiII_1304_dust : extracting velocities from ramses '
       allocate(v(3,nleaf))
       ...
       write(*,*) '-- module_gas_composition_SiII_1304_dust : extracting nHI and T from ramses '
       ...
       g(:)%dopwidth = sqrt((2.0d0*kb/mp/28.085)*T) ! [ cm/s ]   !28.085 is the atomic mass of silicone

       ! get ndust (pseudo dust density from Laursen, Sommer-Larsen, Andersen 2009)
       write(*,*) '-- module_gas_composition_SiII_1304_dust : extracting ndust from ramses '
  ...
  function  gas_get_scatter_flag(cell_gas, distance_to_border_cm, nu_cell, tau_abs,iran)
    ...
    ! - gas_get_scatter_flag : 0 [no scatter], 1 [SiII-1304 scatter], 2 [dust] 
    ! --------------------------------------------------------------------------
    ...
    real(kind=8)                          :: tau_SiII_1304, tau_dust, tau_cell, proba04, x
    
    ! compute optical depths for different components of the gas.
    tau_SiII_1304 = get_tau_SiII_1304(cell_gas%nSiII, cell_gas%dopwidth, distance_to_border_cm, nu_cell)
    tau_dust      = get_tau_dust(cell_gas%ndust, distance_to_border_cm, nu_cell)
    tau_cell      = tau_SiII_1304 + tau_dust
    ...
    if (tau_abs > tau_cell) then  ! photon is due for absorption outside the cell 
       ...
       ! decide if it is 1304 or dust 
       proba04 = tau_SiII_1304 /  tau_cell
       x = ran3(iran)
       if (x <= proba04) then
          gas_get_scatter_flag = 1 ! absorption by SiII-1304
       else
  ...
  function  gas_get_tau(cell_gas, distance_cm, nu_cell)
    ...
    ! compute optical depths for different components of the gas.
    tau_SiII   = get_tau_SiII_1304(cell_gas%nSiII, cell_gas%dopwidth, distance_cm, nu_cell)
  ...
  function gas_peeloff_weight(flag,cell_gas,nu_ext,kin,kout,iran)
    ...
    select case(flag)
    case(1)
       gas_peeloff_weight = SiII_1304_peeloff_weight(cell_gas%v, cell_gas%dopwidth, nu_ext, kin, kout, iran)
    case(2)
       gas_peeloff_weight = dust_peeloff_weight(cell_gas%v, nu_ext, kin, kout)
    case default
       print*,'ERROR in module_gas_composition_SiII1304_dust.f90:gas_peeloff_weight - unknown case : ',flag 
       stop
    end select
  ...
  !--PIKSEROC--
    ...
    select case(flag)
    case(1)
       call scatter_SiII_1304(cell_gas%v, cell_gas%dopwidth, nu_cell, k, nu_ext, iran)
    case(2)
