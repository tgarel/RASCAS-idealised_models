module module_gas_composition
  ...
  !  HI and HII gas, (mainly to compute the column densities of HI and HII)
  ! - The contents are derived from RAMSES directly
  ...
  type, public :: gas
     ...
     real(kind=8) :: nHII      ! HII numerical density [HI/cm3]
...
contains
    ...
    if (verbose) print*,'min/max of nHI : ',minval(gas_leaves(:)%nHI),maxval(gas_leaves(:)%nHI)
    if (verbose) print*,'min/max of nHII : ',minval(gas_leaves(:)%nHII),maxval(gas_leaves(:)%nHII)
  ...
  subroutine gas_from_ramses_leaves(repository,snapnum,nleaf,nvar,ramses_var, g)
    ...
    real(kind=8),dimension(:),allocatable          :: T, nhi, nh
  ...
  function gas_get_CD(cell_gas, distance_cm)
    ...
    gas_get_CD = (/ distance_cm*cell_gas%nHI, distance_cm*cell_gas%nHII /)
