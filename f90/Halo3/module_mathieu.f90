...
!$OMP END PARALLEL
    ...
    open(unit=14, file='result_Halo3_nside24_lum_group.txt', status='replace', form='formatted', action='write')
    do j=1,ndirections
       write(14,*) fesc(j)
    enddo 
    close(14)
    open(unit=14, file='nH_Halo3_nside24.txt', status='replace', form='formatted', action='write')
    do j=1,ndirections
       write(14,*) densh(j)
    enddo
    close(14)    
    open(unit=14, file='dist_Halo3_nside24.txt', status='replace', form='formatted', action='write')
    do j=1,ndirections
       write(14,*) distance(j)
    enddo
    close(14)
