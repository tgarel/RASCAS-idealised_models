module module_opt_ray
  ...
  type ray_type
     ...
     real(kind=8),allocatable  :: E(:)     ! energy in EV
     real(kind=8)              :: dist     ! distance traveled along ray (box units)
     real(kind=8),allocatable  :: tau(:)   ! integrated opacity along ray
  ...
  type ray_type_CD
     integer(kind=4)           :: ID       ! a positive unique ID
     real(kind=8)              :: dist     ! distance traveled along ray (box units)
     real(kind=8),allocatable  :: CD(:)       ! integrated column density along ray
     real(kind=8),dimension(3) :: x_em     ! emission location (box units)
     real(kind=8),dimension(3) :: k_em     ! emission direction == propagation direction (normalised vector)
  end type ray_type_CD
  ...
  public  :: init_rays_from_file, init_halos_from_file, dump_rays, ray_advance, ray_advance_CD
  private :: path

contains


  subroutine ray_advance(ray,domesh,domaine_calcul,maxdist)
    ...
    real(kind=8),allocatable       :: tau_cell(:), tau(:)
    real(kind=8)                   :: dist, dborder, d
    integer(kind=4)                :: i, icellnew, npush, nSEDgroups
    ...
    nSEDgroups = size(ray%E)
    allocate(tau(nSEDgroups), tau_cell(nSEDgroups))
    ...
    ray_propagation : do
       ...
       do i=1,nSEDgroups
          tau_cell(i) = gas_get_tau_gray(cell_gas, distance_to_border_cm, ray%E(i)*evtoerg/planck)
       end do
...
!!$ ppos(3) = ppos(3) + merge(-1.0d0,1.0d0,kray(3)<0.0d0) * epsilon(ppos(3))
  ...
  subroutine ray_advance_CD(ray,domesh,domaine_calcul,maxdist)

    type(ray_type_CD),intent(inout):: ray            ! a ray 
    type(mesh),intent(in)          :: domesh         ! mesh
    type(domain),intent(in)        :: domaine_calcul ! domaine dans lequel on propage les photons...
    real(kind=8),intent(in)        :: maxdist      ! stop propagation after a distance of maxdist    
    type(gas)                      :: cell_gas       ! gas in the current cell 
    ...
    real(kind=8),dimension(3)      :: ppos,ppos_cell ! working coordinates of photon (in box and in cell units)
    real(kind=8)                   :: distance_to_border,distance_to_border_cm
    real(kind=8),allocatable       :: CD_cell(:), CD(:)
    real(kind=8)                   :: dist, dborder, d
    integer(kind=4)                :: i, icellnew, npush, nCD
    real(kind=8),dimension(3)      :: vgas, kray, cell_corner, posoct
    logical                        :: flagoutvol, in_domain
    real(kind=8)                   :: excess, tmp
    ...
    nCD = gas_get_n_CD()
    allocate(CD_cell(nCD), CD(nCD))
    CD  = 0.0d0      ! corresponding column density
    maxdist_cm = maxdist * box_size_cm ! dist_tot is now provided in cm ...
    ...
    ray_propagation : do
       ...
       ! compute (total) column density along ray in cell
       CD_cell(:) = gas_get_CD(cell_gas, distance_to_border_cm)
       ...
       if (dist > maxdist_cm) then
          ...
          ray%CD(:)  = CD(:) - (excess / distance_to_border_cm)*CD_cell(:)
          exit ray_propagation
       endif
