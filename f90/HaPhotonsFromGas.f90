program HaPhotonsFromGas
  ...
  real(kind=8),parameter   :: lambda_0=6562.7d0                           ![A] Ha wavelength
  ...
  real(kind=8),allocatable     :: x_leaf(:,:),ramses_var(:,:),HaEm(:),v_leaf(:,:),HIDopWidth(:),cell_volume_vs_level(:)
  real(kind=8)                 :: r1, r2, dx, dv, nu, scalar, recomb_total,k(3), boxsize,maxrec
  real(kind=8)                 :: start_photpacket,end_photpacket,x(3),xmin,xmax,ymin,ymax,zmin,zmax
  logical                      :: ok 
  real(kind=8),allocatable :: nu_em(:),x_em(:,:),k_em(:,:),nu_cell(:)
  integer(kind=4),dimension(:),allocatable :: cpu_list
  integer(kind=4) :: ncpu_read
  real(kind=8),allocatable :: low_prob_rec(:)
  ...
  ! ---------------------------------------------------------------------------
  ! --- input / outputs
  character(2000)           :: outputfile = 'HaPhotIC' ! file to which recombination photons will be written
  ...
  allocate(HaEm(nsel),HIDopWidth(nsel))
  call ramses_get_HaEmiss_HIDopwidth(repository,snapnum,nleaftot,nvar,ramses_var,HaEm,HIDopWidth,sample=emitting_cells)
  ...
  do i=1,nsel
     ...
     HaEm(i) = HaEm(i) * dv   ! erg/s 
     if (HaEm(i) > maxrec) maxrec = HaEm(i)
  end do  
  recomb_total = sum(HaEm) / (planck*nu_0)  ! nb of photons per second
  print*,'total Ha luminosity [erg/s] = ',recomb_total*(planck*nu_0)

  HaEm = HaEm / maxrec
  allocate(low_prob_rec(nsel+1))
  low_prob_rec(1) = 0.0d0
  do i = 2,nsel
     low_prob_rec(i) = low_prob_rec(i-1) + HaEm(i-1) 
  end do
  low_prob_rec = low_prob_rec / (low_prob_rec(nsel)+HaEm(nsel))
  ...
  ! write ICs
  open(unit=14, file=outputfile, status='unknown', form='unformatted', action='write')
...
contains
    ...
    do
       ...
       if(err/=0) exit
       if (line(1:18) == '[HaPhotonsFromGas]') then
