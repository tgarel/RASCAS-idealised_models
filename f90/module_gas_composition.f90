module module_gas_composition
  ...
  !laV
  public :: ion_number, mix_H_metals
...
contains
  ...
  function gas_get_scatter_flag(cell_gas, distance_to_border_cm, nu_cell, tau_abs,iran)
    ...
    integer(kind=4)                       :: gas_get_scatter_flag, i!, ion
    ...
    if (tau_abs > tau_cell) then  ! photon is due for absorption outside the cell 
       ...
       if(tirage < tau_dust/tau_cell) then
          ...
          do i=1,n_line_tot
             !ion = get_ion_index(i)
             ! tau = tau + get_tau_single_line(ion, i, cell_gas%n(ion), cell_gas%dopwidth, distance_to_border_cm, nu_cell)
