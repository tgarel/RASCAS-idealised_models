module module_idealised_models
  ...
  ! public functions:
  public :: read_IdealisedModels_params, print_IdealisedModels_params, compute_idealised_gas, shell_V_rho_gradient, shellcone_V_rho_gradient, shell_chisholm, shell_V_rho_gradient_steady, sphere_homogen_velfix, sphere_homogen_Vgradient, shell_starburst_rho_gradient, shell_Vcak_rho_gradient, shell_Vcak_rho_steady, shell_Vdecrease_rho_gradient
...
contains
  ...
  subroutine compute_idealised_gas(n_dust,n_gas,b_param,v_leaf,x_leaf,leaf_level,nleaf)
    ...
    case('shell_Vdecrease_rho_gradient')
       do ileaf=1,nleaf
          call shell_Vdecrease_rho_gradient(n_dust(ileaf),n_gas(ileaf),b_param(ileaf),v_leaf(1,ileaf),v_leaf(2,ileaf),v_leaf(3,ileaf),x_leaf(ileaf,1),x_leaf(ileaf,2),x_leaf(ileaf,3),dx_cell)
       end do
    ...
    case('shell_Vcak_rho_gradient')
       do ileaf=1,nleaf
          call shell_Vcak_rho_gradient(n_dust(ileaf),n_gas(ileaf),b_param(ileaf),v_leaf(1,ileaf),v_leaf(2,ileaf),v_leaf(3,ileaf),x_leaf(ileaf,1),x_leaf(ileaf,2),x_leaf(ileaf,3),dx_cell)
       end do
    case('shell_Vcak_rho_steady')
       do ileaf=1,nleaf
          call shell_Vcak_rho_steady(n_dust(ileaf),n_gas(ileaf),b_param(ileaf),v_leaf(1,ileaf),v_leaf(2,ileaf),v_leaf(3,ileaf),x_leaf(ileaf,1),x_leaf(ileaf,2),x_leaf(ileaf,3),dx_cell)
       end do
    end select
...
!+++++++++++++++++++++++++++++++++++++++++++++++ SHELL with velocity from r^-2 force and power-law density gradient +++++++++++++++++++++++++++++++++++++++++++++
   ...
   subroutine shell_starburst_rho_gradient(ndust_ideal,ngas_ideal,bparam_ideal,vx_ideal,vy_ideal,vz_ideal,xcell_ideal,ycell_ideal,zcell_ideal,dx_cell)
    ...
    !real(kind=8),parameter                :: vmax_alpha_normed = 0.8262 ! assuming alpha=3.8 ! = 1./sqrt(alpha-1.0-ln(alpha))
    real(kind=8)                          :: vmax_alpha_normed 


    vmax_alpha_normed = 1.0d0/sqrt(alpha-1.0d0-log(alpha))
...
!+++++++++++++++++++++++++++++++++++++++++++++++ SHELL with velocity gradient and density gradient (power-laws) +++++++++++++++++++++++++++++++++++++++++++++
  ...
  subroutine shell_Vdecrease_rho_gradient(ndust_ideal,ngas_ideal,bparam_ideal,vx_ideal,vy_ideal,vz_ideal,xcell_ideal,ycell_ideal,zcell_ideal,dx_cell)
    
    implicit none

    ! Shell (or sphere if r_min=0) with "power-law" rho profile and linearly-decreasing Velocity
    ! point source at center and medium transparent at R < r_min and at R > r_max
    ! V(r) = Vmax * (rmax-r)/(rmax-rmin)
    ...
    if (MCsampling) then
       ...
       vx_ideal = Vgas_norm * (r_max-dist_cell) / (r_max-r_min) * (xcell_ideal - 0.5d0) / dist_cell
       vy_ideal = Vgas_norm * (r_max-dist_cell) / (r_max-r_min) * (ycell_ideal - 0.5d0) / dist_cell
       vz_ideal = Vgas_norm * (r_max-dist_cell) / (r_max-r_min) * (zcell_ideal - 0.5d0) / dist_cell
  ...
  end subroutine shell_Vdecrease_rho_gradient
...
!!$          vz_ideal    = Vgas_norm * (zcell_ideal-0.5d0) / r_max
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++ SHELL with CAK velocity gradient and density gradient +++++++++++++++++++++++++++++++++++++++++++++

  subroutine shell_Vcak_rho_gradient(ndust_ideal,ngas_ideal,bparam_ideal,vx_ideal,vy_ideal,vz_ideal,xcell_ideal,ycell_ideal,zcell_ideal,dx_cell)
    
    implicit none

    ! Shell (or sphere if r_min=0) with CAK V and rho "power-law" profiles 
    ! point source at center and medium transparent at R < r_min and at R > r_max
    ! V(r) = Vmax * (1- r_min/r)**beta, beta CANNOT be 1 
    ...
    integer(kind=4)                       :: missed_cell
    real(kind=8)                          :: n0, coldens_dust, ndust_0
    real(kind=8),parameter                :: beta=0.5
    ...
    if (MCsampling) then
       ...
       vx_ideal = Vgas_norm * (1.0d0 - r_min/dist_cell)**beta * (xcell_ideal - 0.5d0) / dist_cell    
       vy_ideal = Vgas_norm * (1.0d0 - r_min/dist_cell)**beta * (ycell_ideal - 0.5d0) / dist_cell  
       vz_ideal = Vgas_norm * (1.0d0 - r_min/dist_cell)**beta * (zcell_ideal - 0.5d0) / dist_cell  
  ...
  end subroutine shell_Vcak_rho_gradient

   
  !+++++++++++++++++++++++++++++++++++++++++++++++ SHELL with CAK velocity gradient and density gradient +++++++++++++++++++++++++++++++++++++++++++++

  subroutine shell_Vcak_rho_steady(ndust_ideal,ngas_ideal,bparam_ideal,vx_ideal,vy_ideal,vz_ideal,xcell_ideal,ycell_ideal,zcell_ideal,dx_cell)
    
    implicit none

    ! Shell (or sphere if r_min=0) with CAK V and rho "steady-state" profile 
    ! point source at center and medium transparent at R < r_min and at R > r_max
    ! V(r) = Vmax * (1- r_min/r)**beta, beta CANNOT be 1 
    ...
    if (coldens_norm .gt. 1.0d0) then
       ! Works only for beta=0.5
       n0 = coldens_norm * r_min * box_size_IM_cm / 2.0d0 / (1.0d0 - r_min/r_max)**0.5  ! cm-3
    else
       ! I use the std density ngas_norm
       n0 = ngas_norm 
    end if
    ...
    if (MCsampling) then
       ...
       ngas_ideal  = n0 * 1.0d0 /  (dist_cell**2. * (1.0d0 - r_min/dist_cell)**beta) * volfrac2
       ...
       if (dist_cell < r_max .and. dist_cell > r_min) then ! cell completely within sphere
          ...
          ngas_ideal  = n0 * 1.0d0 /  (dist_cell**2. * (1.0d0 - r_min/dist_cell)**beta)
          ndust_ideal = ndust_0 * ngas_ideal / n0
          missed_cell = 0
  ...
  end subroutine shell_Vcak_rho_steady
...
!!$          vz_ideal    = Vgas_norm * (zcell_ideal-0.5d0) / r_max
  ...
  subroutine sphere_homogen_velfix(ndust_ideal,ngas_ideal,bparam_ideal,vx_ideal,vy_ideal,vz_ideal,xcell_ideal,ycell_ideal,zcell_ideal,dx_cell)
   ...
   !!+++++++++++++++++++++++++++++ HOMOGEN Sphere with V gradient ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  
  subroutine sphere_homogen_Vgradient(ndust_ideal,ngas_ideal,bparam_ideal,vx_ideal,vy_ideal,vz_ideal,xcell_ideal,ycell_ideal,zcell_ideal,dx_cell)
  ...
  end subroutine sphere_homogen_Vgradient
