module module_FeII_2374_model ! Fe_II UV2

  ! This module describes the absorption of photons by FeII from level  3d^6 4s 6D 9/2 to level 3d^6 4p 6F^0 9/2.
  ! This transition is at 2374.46 A. (named 2374)
  ! The module also implements the two decay channels (resonant and fluorescent) at 2374.46 A and 2396.35 A. 
  ...
  ! level 3 is 3d^6 4p 6F^0 9/2

  ! transition between levels 1 and 3
  real(kind=8),parameter :: lambda13       = 2374.46d0                    ! transition wavelength [A]
  real(kind=8),parameter :: lambda13_cm    = lambda13 / cmtoA             ! [cm]
  real(kind=8),parameter :: nu13           = clight / lambda13_cm         ! [Hz]
  real(kind=8),parameter :: f13            = 3.59d-2                      ! oscillator strength
  real(kind=8),parameter :: sigma13_factor = sqrtpi*e_ch**2*f13/me/clight ! multiply by Voigt(x,a)/delta_nu_doppler to get sigma.
  real(kind=8),parameter :: A31            = 4.25d7                       ! spontaneous decay [/s]

  ! transition between levels 3 and 2
  real(kind=8),parameter :: lambda23       = 2396.35d0                    ! transition wavelength [A]
  real(kind=8),parameter :: lambda23_cm    = lambda23 / cmtoA             ! [cm]
  real(kind=8),parameter :: nu23           = clight / lambda23_cm         ! [Hz]
  real(kind=8),parameter :: A32            = 2.59d8                       ! spontaneous decay [/s]

  real(kind=8),parameter :: A31_over_A31_plus_A32 = A31 / (A31+A32)
  
  public :: get_tau_FeII_2374, scatter_FeII_2374, read_FeII_2374_params, print_FeII_2374_params

contains

  function get_tau_FeII_2374(nFeII, vth, distance_to_border_cm, nu_cell)

    ! --------------------------------------------------------------------------
    ! compute optical depth of FeII-2374.46 over a given distance
    ! --------------------------------------------------------------------------
    ! INPUTS:
    ...
    ! OUTPUT :
    ! - get_tau_FeII_2374 : optical depth of Fer's line over distance_to_border_cm
    ! --------------------------------------------------------------------------
    
    real(kind=8),intent(in) :: nFeII,vth,distance_to_border_cm,nu_cell
    real(kind=8)            :: delta_nu_doppler,x_cell,sigma,a,h_cell,get_tau_FeII_2374

    ! compute Doppler width and a-parameter
    delta_nu_doppler = vth / lambda13_cm
    a = A31 / (fourpi * delta_nu_doppler)

    ! cross section of FeII-2374.46
    x_cell = (nu_cell - nu13) / delta_nu_doppler
    h_cell = voigt_function(x_cell,a)
    sigma  = sigma13_factor / delta_nu_doppler * h_cell

    get_tau_FeII_2374 = sigma * nFeII * distance_to_border_cm
   
    return

  end function get_tau_FeII_2374

  
  subroutine scatter_FeII_2374(vcell,vth,nu_cell,k,nu_ext,iran)
  ...
  end subroutine scatter_FeII_2374


  subroutine read_FeII_2374_params(pfile)
  ...
  end subroutine read_FeII_2374_params



  subroutine print_FeII_2374_params(unit)
  ...
  end subroutine print_FeII_2374_params

  
end module module_FeII_2374_model
